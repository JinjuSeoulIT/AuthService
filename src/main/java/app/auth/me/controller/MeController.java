...
public class MeController {
    ...
    public ResponseEntity<ApiResponse<AuthUserInfo>> me() {
        ...
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new ApiResponse<AuthUserInfo>().error("AUTH_UNAUTHORIZED"));
        }
        ...
        return ResponseEntity.ok(new ApiResponse<AuthUserInfo>().ok(userInfo));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> changeMyPassword(@RequestBody ChangePasswordRequest request) {
        ...
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new ApiResponse<Void>().error("AUTH_UNAUTHORIZED"));
        }

        meService.changeMyPassword(authentication.getName(), request);
        return ResponseEntity.ok(new ApiResponse<Void>().ok("AUTH_PASSWORD_CHANGED"));
    }
}
