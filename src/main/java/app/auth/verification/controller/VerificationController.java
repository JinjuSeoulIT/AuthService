...
public class VerificationController {
    ...
    public ResponseEntity<ApiResponse<Void>> sendRegisterEmailCode(@RequestBody RegisterContactSendRequest request) {
        try {
            return ResponseEntity.ok(new ApiResponse<Void>().ok(verificationService.sendRegisterEmailCode(request.getValue())));
    ...
    public ResponseEntity<ApiResponse<Void>> sendRegisterPhoneCode(@RequestBody RegisterContactSendRequest request) {
        try {
            return ResponseEntity.ok(new ApiResponse<Void>().ok(verificationService.sendRegisterPhoneCode(request.getValue())));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ApiResponse<Void>().error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> sendVerificationEmail(@RequestBody EmailSendRequest request, Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ApiResponse<Void>().error("AUTH_UNAUTHORIZED"));
        }
        try {
            return ResponseEntity.ok(new ApiResponse<Void>().ok(verificationService.sendVerificationCode(authentication.getName(), request.getEmail())));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ApiResponse<Void>().error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Boolean>> verifyEmailCode(@RequestBody EmailVerifyRequest request, Authentication authentication) {
        ...
        try {
            ...
            if (!verified) {
                return ResponseEntity.badRequest().body(new ApiResponse<Boolean>().error("AUTH_EMAIL_CODE_MISMATCH"));
            }
    ...
    public ResponseEntity<ApiResponse<Void>> sendVerificationPhone(@RequestBody PhoneSendRequest request, Authentication authentication) {
        ...
        try {
            return ResponseEntity.ok(new ApiResponse<Void>().ok(verificationService.sendPhoneVerificationCode(authentication.getName(), request.getPhone())));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ApiResponse<Void>().error(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Boolean>> verifyPhoneCode(@RequestBody PhoneVerifyRequest request, Authentication authentication) {
        ...
        try {
            ...
            if (!verified) {
                return ResponseEntity.badRequest().body(new ApiResponse<Boolean>().error("AUTH_PHONE_CODE_MISMATCH"));
            }
            return ResponseEntity.ok(new ApiResponse<Boolean>().ok(true));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ApiResponse<Boolean>().error(e.getMessage()));
